...
class SHA:
    ...
    @staticmethod
    def convertBin(text: str):
        # char -> ascii -> binary -> strip 0b -> convert to hexadecimal -> strip 0x -> append to new string
        binary = ''
        for letter in text:
            ascii = ord(letter)
            ascii_bin = bin(ascii)
            ascii_bin = ascii_bin.removeprefix('0b')
            binary = binary + ascii_bin

        return binary

    @staticmethod
    def padding(text: str):
        # This only works on small text for now (if message > 512 then we'll have to modify this a bit)
        padded_text = text + "1"  # Append one 1
        # then append 0 until padded text length is 448
        while (len(padded_text) != 448):
            padded_text = padded_text + '0'

        return padded_text

    # TODO: Some complicate math what even is this?
